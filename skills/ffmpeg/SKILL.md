...
- **Cấu trúc thư mục con bắt buộc**:
  ...
  - `scripts/`: Chứa các script batch, script bash phụ trợ cho phiên làm việc (ví dụ `export-all-platforms.sh`).
  - `temp/`: Chứa file danh sách ghép nối (`list.txt`), palette gen tạm, audio tách rời tạm, video trung gian.
- **Quy tắc cấm xả rác**: Tuyệt đối không lưu file trực tiếp ra thư mục gốc (`cwd` / repo root), ví dụ: `output.mp4`, `list.txt`, `trimmed.mp4`, `out/`, `public/demos/`. Mọi câu lệnh bên dưới phải luôn gắn liền với biến `$SESSION_DIR`.
...
```bash
ffmpeg -i "$SESSION_DIR/input/input.gif" -movflags faststart -pix_fmt yuv420p \
  -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" "$SESSION_DIR/output/output.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/input.mp4" -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2" "$SESSION_DIR/output/output.mp4"

# To 1920x1080 (crop to fill)
ffmpeg -i "$SESSION_DIR/input/input.mp4" -vf "scale=1920:1080:force_original_aspect_ratio=increase,crop=1920:1080" "$SESSION_DIR/output/output.mp4"

# Scale to width, auto height
ffmpeg -i "$SESSION_DIR/input/input.mp4" -vf "scale=1280:-2" "$SESSION_DIR/output/output.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/input.mp4" -c:v libx264 -crf 23 -preset medium -c:a aac -b:a 128k "$SESSION_DIR/output/output.mp4"

# Aggressive compression for web preview
ffmpeg -i "$SESSION_DIR/input/input.mp4" -c:v libx264 -crf 28 -preset fast -c:a aac -b:a 96k "$SESSION_DIR/output/output.mp4"

# Target file size (e.g., ~10MB for 60s video = ~1.3Mbps)
ffmpeg -i "$SESSION_DIR/input/input.mp4" -c:v libx264 -b:v 1300k -c:a aac -b:a 128k "$SESSION_DIR/output/output.mp4"
```
...
```bash
# Extract to MP3
ffmpeg -i "$SESSION_DIR/input/input.mp4" -vn -acodec libmp3lame -q:a 2 "$SESSION_DIR/output/output.mp3"

# Extract to AAC
ffmpeg -i "$SESSION_DIR/input/input.mp4" -vn -acodec aac -b:a 192k "$SESSION_DIR/output/output.m4a"

# Extract to WAV (uncompressed)
ffmpeg -i "$SESSION_DIR/input/input.mp4" -vn "$SESSION_DIR/output/output.wav"
```
...
ffmpeg -i "$SESSION_DIR/input/input.m4a" -codec:a libmp3lame -qscale:a 2 "$SESSION_DIR/output/output.mp3"

# WAV to MP3
ffmpeg -i "$SESSION_DIR/input/input.wav" -codec:a libmp3lame -b:a 192k "$SESSION_DIR/output/output.mp3"

# Adjust volume
ffmpeg -i "$SESSION_DIR/input/input.mp3" -filter:a "volume=1.5" "$SESSION_DIR/output/output.mp3"
```
...
ffmpeg -i "$SESSION_DIR/input/input.mp4" -ss 00:00:30 -t 00:00:15 -c:v libx264 -c:a aac "$SESSION_DIR/output/output.mp4"

# Cut from timestamp to timestamp
ffmpeg -i "$SESSION_DIR/input/input.mp4" -ss 00:00:30 -to 00:00:45 -c:v libx264 -c:a aac "$SESSION_DIR/output/output.mp4"

# Stream copy (faster but may lose frames at cut points)
# Only use when source has frequent keyframes
ffmpeg -i "$SESSION_DIR/input/input.mp4" -ss 00:00:30 -t 00:00:15 -c copy "$SESSION_DIR/output/output.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/input.mp4" -filter_complex "[0:v]setpts=0.5*PTS[v];[0:a]atempo=2.0[a]" -map "[v]" -map "[a]" "$SESSION_DIR/output/output.mp4"

# 0.5x speed (slow motion)
ffmpeg -i "$SESSION_DIR/input/input.mp4" -filter_complex "[0:v]setpts=2.0*PTS[v];[0:a]atempo=0.5[a]" -map "[v]" -map "[a]" "$SESSION_DIR/output/output.mp4"

# Video only (no audio)
ffmpeg -i "$SESSION_DIR/input/input.mp4" -filter:v "setpts=0.5*PTS" -an "$SESSION_DIR/output/output.mp4"
```

### Concatenate Videos

```bash
# Create file list in temp
echo "file '$SESSION_DIR/input/clip1.mp4'" > "$SESSION_DIR/temp/list.txt"
echo "file '$SESSION_DIR/input/clip2.mp4'" >> "$SESSION_DIR/temp/list.txt"
echo "file '$SESSION_DIR/input/clip3.mp4'" >> "$SESSION_DIR/temp/list.txt"

# Concatenate (same codec/resolution)
ffmpeg -f concat -safe 0 -i "$SESSION_DIR/temp/list.txt" -c copy "$SESSION_DIR/output/output.mp4"

# Concatenate with re-encoding (different sources)
ffmpeg -f concat -safe 0 -i "$SESSION_DIR/temp/list.txt" -c:v libx264 -c:a aac "$SESSION_DIR/output/output.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/input.mp4" -vf "fade=t=in:st=0:d=1,fade=t=out:st=9:d=1" -c:a copy "$SESSION_DIR/output/output.mp4"

# Audio fade
ffmpeg -i "$SESSION_DIR/input/input.mp4" -af "afade=t=in:st=0:d=1,afade=t=out:st=9:d=1" -c:v copy "$SESSION_DIR/output/output.mp4"
```
...
ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "$SESSION_DIR/input/input.mp4"

# Full info
ffprobe -v quiet -print_format json -show_format -show_streams "$SESSION_DIR/input/input.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/demo-raw.mp4" \
  -filter_complex "[0:v]setpts=0.333*PTS[v];[0:a]atempo=3.0[a]" \
  -map "[v]" -map "[a]" \
  "$SESSION_DIR/output/demo-fast.mp4"

# Slow motion for emphasis (0.5x speed)
ffmpeg -i "$SESSION_DIR/input/action.mp4" \
  -filter_complex "[0:v]setpts=2.0*PTS[v];[0:a]atempo=0.5[a]" \
  -map "[v]" -map "[a]" \
  "$SESSION_DIR/output/action-slow.mp4"

# Speed up without audio (common for screen recordings)
ffmpeg -i "$SESSION_DIR/input/demo.mp4" -filter:v "setpts=0.5*PTS" -an "$SESSION_DIR/output/demo-2x.mp4"

# Timelapse effect (10x speed, drop audio)
ffmpeg -i "$SESSION_DIR/input/long-demo.mp4" -filter:v "setpts=0.1*PTS" -an "$SESSION_DIR/output/timelapse.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/raw-recording.mp4" \
  -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,fps=30" \
  -c:v libx264 -crf 18 -preset slow \
  -c:a aac -b:a 192k \
  -movflags faststart \
  "$SESSION_DIR/output/demo.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/iphone-recording.mov" \
  -vf "scale=1920:-2,fps=30" \
  -c:v libx264 -crf 20 \
  -an \
  "$SESSION_DIR/output/mobile-demo.mp4"
```

### Batch Convert GIFs

```bash
for f in "$SESSION_DIR/input/"*.gif; do
  ffmpeg -i "$f" -movflags faststart -pix_fmt yuv420p \
    -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" \
    "$SESSION_DIR/output/$(basename "$f" .gif).mp4"
done
```
...
After Remotion renders your video (typically placed at `$SESSION_DIR/input/video.mp4` or `$SESSION_DIR/temp/video.mp4`), use FFmpeg to optimize for each distribution platform into `$SESSION_DIR/output/`.
...
Remotion render (master)                FFmpeg optimization                         Platform upload
       ↓                                       ↓                                          ↓
$SESSION_DIR/input/video.mp4  ────────→  $SESSION_DIR/output/video-youtube.mp4   ───→  YouTube
                             ────────→  $SESSION_DIR/output/video-twitter.mp4   ───→  Twitter/X
                             ────────→  $SESSION_DIR/output/video-linkedin.mp4  ───→  LinkedIn
                             ────────→  $SESSION_DIR/output/video-web.mp4       ───→  Website embed
```
...
ffmpeg -i "$SESSION_DIR/input/video.mp4" \
  -c:v libx264 -preset slow -crf 18 \
  -profile:v high -level 4.0 \
  -bf 2 -g 30 \
  -c:a aac -b:a 192k -ar 48000 \
  -movflags +faststart \
  "$SESSION_DIR/output/video-youtube.mp4"

# YouTube Shorts (vertical 1080x1920)
ffmpeg -i "$SESSION_DIR/input/video.mp4" \
  -vf "scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2" \
  -c:v libx264 -crf 18 -c:a aac -b:a 192k \
  "$SESSION_DIR/output/video-shorts.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/video.mp4" \
  ...
  -movflags +faststart \
  -fs 15M \
  "$SESSION_DIR/output/video-twitter.mp4"

# Check file size and duration
ffprobe -v error -show_entries format=duration,size -of csv=p=0 "$SESSION_DIR/output/video-twitter.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/video.mp4" \
  ...
  -c:a aac -b:a 192k -ar 48000 \
  -movflags +faststart \
  "$SESSION_DIR/output/video-linkedin.mp4"
```
...
ffmpeg -i "$SESSION_DIR/input/video.mp4" \
  ...
  -c:a aac -b:a 128k \
  -movflags +faststart \
  "$SESSION_DIR/output/video-web.mp4"
...
ffmpeg -i "$SESSION_DIR/input/video.mp4" \
  ...
  -deadline good \
  "$SESSION_DIR/output/video-web.webm"
```
...
ffmpeg -i "$SESSION_DIR/input/video.mp4" -t 5 \
  -vf "fps=15,scale=480:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" \
  "$SESSION_DIR/output/preview.gif"

# Smaller file GIF
ffmpeg -i "$SESSION_DIR/input/video.mp4" -t 3 \
  -vf "fps=10,scale=320:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" \
  "$SESSION_DIR/output/preview-small.gif"
```
...
ffmpeg -i "$SESSION_DIR/input/input.mp4" -c:v libx264 "$SESSION_DIR/output/output.mp4" && echo "Success" || echo "Failed: check input file"

# Validate output file is playable
ffprobe -v error -select_streams v:0 -show_entries stream=codec_name -of csv=p=0 "$SESSION_DIR/output/output.mp4"

# Get detailed error info
ffmpeg -v error -i "$SESSION_DIR/input/input.mp4" -f null - 2>&1 | head -20
```

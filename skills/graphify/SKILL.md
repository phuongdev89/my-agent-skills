...
- **Cấu trúc thư mục con bắt buộc**:
  ...
  - `scripts/`: Chứa các script bổ trợ tạm cho session (custom AST runner, filter script, hook test).
  - `temp/`: Chứa các file trung gian nội bộ (`.graphify_detect.json`, `.graphify_ast.json`, chunk files, cache...).
- **Quy tắc cấm xả rác**: Tuyệt đối không xả file tài liệu tải về, file chunk hay file export ra root repo (`cwd`). Mọi output và export phải lưu vào `output/` của session dir.
...
/graphify <path> --neo4j                              # generate cypher.txt for Neo4j in $SESSION_DIR/output/
/graphify <path> --neo4j-push bolt://localhost:7687   # push directly to Neo4j
/graphify <path> --falkordb                           # generate cypher.txt for FalkorDB in $SESSION_DIR/output/
/graphify <path> --falkordb-push falkordb://localhost:6379   # push directly to FalkorDB
/graphify <path> --mcp                                # start MCP stdio server for agent access
/graphify <path> --watch                              # watch folder, auto-rebuild on code changes (no LLM needed)
/graphify <path> --wiki                               # build agent-crawlable wiki (index.md + one article per community)
/graphify <path> --obsidian --obsidian-dir $SESSION_DIR/output/obsidian  # write vault to session output path
/graphify add <url>                                   # fetch URL, save to $SESSION_DIR/raw, update graph
...
# Đồng bộ artifacts hoàn chỉnh vào session output directory:
cp -r .graphify-out/graph.html .graphify-out/GRAPH_REPORT.md .graphify-out/graph.json "$SESSION_DIR/output/" 2>/dev/null || true
if [ -d .graphify-out/obsidian ]; then cp -r .graphify-out/obsidian "$SESSION_DIR/output/"; fi
```
...
```
Graph complete. Outputs copied to session directory: $SESSION_DIR/output/ (and .graphify-out/)

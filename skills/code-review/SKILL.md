...
- **Cấu trúc thư mục con bắt buộc**:
  ...
  - `scripts/`: Chứa script phụ trợ tạm thời (vd: script bóc tách commit, lọc diff, phân tích linter).
  - `temp/`: Chứa diff tạm, log trung gian từ các sub-agent, kết quả kiểm tra tạm thời.
- **Quy tắc cấm xả rác**: Tuyệt đối không xuất file diff (`*.patch`), log tạm thời hay báo cáo ra thư mục gốc (`cwd` / repo root). Mọi thao tác ghi tệp phải trỏ đúng vào session directory đã tạo.

## Process

### 0. Khởi tạo session directory
...
Nếu cần lưu ra file để sub-agents hoặc script phân tích đọc, lưu vào thư mục `temp/` của session:
```bash
git diff <fixed-point>...HEAD > "$SESSION_DIR/temp/changes.diff"
git log <fixed-point>..HEAD --oneline > "$SESSION_DIR/temp/commits.log"
```
...
Lưu báo cáo hoàn chỉnh vào thư mục `output/` của session:
- Đường dẫn: `$SESSION_DIR/output/review-report.md`

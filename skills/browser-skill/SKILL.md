...
- **Cấu trúc phân vùng thư mục con bắt buộc:**
  ...
  └── temp/       # Chứa log, session token cache, dữ liệu tạm
  ```
...
```sh
# Lưu ảnh chụp màn hình vào thư mục session output/ (không xả file ra root)
bsk screenshot --session <id> --out "$SESSION_DIR/output/viewport.png"
bsk screenshot --session <id> --ref @e3 --out "$SESSION_DIR/output/element.png" --json
bsk screenshot --session <id> --full-page --out "$SESSION_DIR/output/page.png"
bsk screenshot --session <id> --full-page --scope current --out "$SESSION_DIR/output/loaded.png"
```
...
```sh
# Upload file từ session_dir/input/ (hoặc downloads/)
bsk upload @e3 --file "$SESSION_DIR/input/report.pdf" --session <id>

# Download file trực tiếp vào session_dir/downloads/ (không xả file ra root)
bsk download @e3 --out "$SESSION_DIR/downloads/report.pdf" --session <id>
```
